...
- **Gaps de segurança/robustez conhecidos:**
  ...
  - ~~WebSockets já abertos não revalidam o token.~~ *(Resolvido no ticket 11: o serviço avisa antes de expirar, o cliente renova pelo mesmo socket, a autorização é revalidada na renovação e a cada minuto, e os três close codes — `1008` unauthenticated, `4401` token expired, `4403` access revoked — são parte do contrato. Ver "A conexão ao vivo" em [`backend/README.md`](backend/README.md).)*
  - **O denylist de revogação urgente não vale para a API, só para os sockets** — um banimento fecha as conexões da pessoa mas o token dela segue lendo por até quinze minutos. Ver "Débito técnico conhecido" em [`backend/README.md`](backend/README.md) para o que isso custaria fechar.
