...
**Known limitation:** ElastiCache is a LocalStack Pro-tier service, so `terraform apply` against LocalStack may not provision it on the free tier. The Terraform module is still written and validated with `terraform plan`; local *application* testing uses the plain Redis container directly, independent of LocalStack.
