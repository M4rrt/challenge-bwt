...
| RDS Postgres instance | Data | `aws_db_instance.postgres` (`rds.tf`) | Single instance, private subnets, not publicly accessible. Holds users/chats/messages. |
...
| CloudFront distribution | Frontend hosting | `aws_cloudfront_distribution.frontend` (`frontend.tf`) | CDN in front of the S3 bucket, serves the SPA to browsers over HTTPS. SPA-aware: 403/404 from S3 are rewritten to `/index.html` (200) so client-side routes like `/chats/123` resolve. |
